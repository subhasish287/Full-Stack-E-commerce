...
const Update = ({ url, token, item, setUpdate, fetchList }) => {
  ...
  const handleSizeChange = (selectedSize, newQuantity) => {
    setSizes((prevSizes) => 
      prevSizes.map((item) =>
        item.size === selectedSize.size ? { ...item, quantity: newQuantity } : item
      )
    );
  };
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(url + "/api/v1/products/update", formData, {
        headers: {
          Authorization: `${token}`,
        },
      });
      ...
      <div className="overlay">
        <div className="modal">
          ...
          <form onSubmit={onSubmitHandler} className="flex-col">
            ...
            <div>
              <p className="product-sizes">Product Sizes and Quantities</p>
              <div className="sizes">
                {sizes.map((size,idx) => (
...
//     setSizes((prev) => {
//       const existingSize = prev.find((item) => item.size === size);
//       if (existingSize) {
//         return prev.map((item) =>
//           item.size === size ? { size, quantity } : item
//         );
//       } else {
...
//       const response = await axios.post(url + "/api/v1/products/update", formData, {
//         headers: {
//           Authorization: token,
//         },
//       });
